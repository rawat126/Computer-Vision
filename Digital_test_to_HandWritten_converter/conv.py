...
# changing our root directory
os.chdir('E:\\alphabets\\')

# function generates final handwritten text image
def fun(xd,length):      
    l2=[]

    # reading all hand-written images into numpy array...
    for i1 in xd:
        l1=[]
        for j1 in i1:
            #print(j1)
    ...
    # joining all images for text generation
    print(length)
    if(length>29):
        return(cv2.blur(cv2.resize(cv2.vconcat(l2),(1320,1520)),(2,2)))

    elif(length<=29 and length>23):
        no_of_spaces = l2[0].shape[1]
        print(no_of_spaces)
        space_list=[]
        for i111 in range(no_of_spaces):
            space_list.append(space)
            
        sp2 = cv2.resize(cv2.hconcat(space_list),(l2[0].shape[1],l2[0].shape[0]))

        l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2)

        return(cv2.blur(cv2.resize(cv2.vconcat(l2),(1320,1230)),(2,2)))

    elif(length<=23 and length>17):
        no_of_spaces = l2[0].shape[1]
        print(no_of_spaces)
        space_list=[]
        for i111 in range(no_of_spaces):
        ...
        return(cv2.blur(cv2.resize(cv2.vconcat(l2),(1320,1050)),(2,2)))

    elif(length<=17 and length>10):
        no_of_spaces = l2[0].shape[1]
        print(no_of_spaces)
        space_list=[]
        for i111 in range(no_of_spaces):
        ...
        l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2)
        l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2)
        l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2)
        print(sp2.shape,'   ',l2[0].shape)

        return(cv2.blur(cv2.resize(cv2.vconcat(l2),(1320,870)),(2,2)))
        ...
        l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2)
        l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2)
        l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2);l2.append(sp2)
        print(sp2.shape,'   ',l2[0].shape)

        return(cv2.blur(cv2.resize(cv2.vconcat(l2),(1320,870)),(2,2)))


# conversion to PDF of generated all handwritten text
def img_to_pdf(root):
    with open('E:\\Assignment.pdf','wb') as ff:
        ff.write(img2pdf.convert(names))
        msg.showinfo('Done...','PDF Saved Sucessfully....')

# conversion to image of generated all handwritten text
def image(s1,e11):
        print(s1.shape)
        cv2.imwrite(e11,s1)
        names.append(e11)
...
# clearing Text boc for next entry
def clear(foot,root,tb):
    tb.delete(1.0,END)

# clearing all GUI windows
def exit1(foot,root):
    foot.destroy()
    root.destroy()
...
# Reading text and identify it character by character....
def pr(root,e11,foot,e1):
    ...
    xd = e1.split('''
''')
    lb = Listbox(foot,height=1,font=(10),width=1,bd=7)
    for i1 in xd:
        lb.insert(END,i1)
        dd.append(len(i1))
    lb.place(x=180,y=135)
    m1 = max(dd)
    length=len(lb.get(0,END))
    lb.destroy()
...
# reading all Upper-case characters handwritten images...... 
...
# reading all Lower-case characters handwritten images......
...
# reading all symbol characters handwritten images......
...
# reading all Numbers handwritten images......
...
#creating GUI Panel(for ease of use)
root = Tk()
